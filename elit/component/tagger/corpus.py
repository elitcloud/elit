    ...
    if isinstance(filename, str):
        with open(filename, encoding='utf-8') as f:
            for line in f:
                split = line.split()
                if len(split) == 2:
                    n, dim = int(split[0]), int(split[1])
                if len(split) > 2:
                    word = split[0]
                    vec = split[1:]
                    if dim == 0:
                        dim = len(vec)
                    elif len(vec) != dim:  # bad line
                        continue
                    word_to_embed[word] = np.array(vec)
    else:
        pret_embeddings = gluonnlp.embedding.create(filename[0], source=filename[1])
        word_to_embed = pret_embeddings
        dim = len(pret_embeddings.idx_to_vec[0])
    return word_to_embed, dim
...
def iob_iobes(tags):
    ...
    for i, tag in enumerate(tags):
        if tag == 'O':
            ...
            raise Exception('Invalid IOB format! ' + str(tags))
    return new_tags
...
if __name__ == '__main__':
    ...
    w2v, dim = read_pretrained_embeddings(('glove', 'glove.6B.100d'))
    pass
