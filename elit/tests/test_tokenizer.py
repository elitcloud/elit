...
@pytest.mark.parametrize('input, offset, expected', [
    ('  Hello\t\r, world  !\n\n', 0,
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['Hello', ',', 'world', '!'], 'off': [(2, 7), (9, 10), (11, 16), (18, 19)]}]}),
    ('  Hello\t\r, world  !\n\n', 1,
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['Hello', ',', 'world', '!'], 'off': [(3, 8), (10, 11), (12, 17), (19, 20)]}]})
])
...
@pytest.mark.parametrize('input, expected', [
    ('', {'doc_id': 0, 'sens': []}),
    (' \t\n\r', {'doc_id': 0, 'sens': []}),
    ('abc', {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['abc'], 'off': [(0, 3)]}]}),
    (' A\tBC  D12 \n', {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['A', 'BC', 'D12'], 'off': [(1, 2), (3, 5), (7, 10)]}]})
])
...
@pytest.mark.parametrize('input, offset, expected', [
    ('Hello, world!', 5,
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['Hello', ',', 'world', '!'], 'off': [(5, 10), (10, 11), (12, 17), (17, 18)]}]}),
])
...
@pytest.mark.parametrize('input, expected', [
    # html entity
    ('ab&larr;cd&#8592;&#x2190;ef&rarr;',
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['ab', '&larr;', 'cd', '&#8592;', '&#x2190;', 'ef', '&rarr;'],
    ...
    ('a:http://ab sftp://ef abeftp://',
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['a', ':', 'http://ab', 'sftp://ef', 'abe', 'ftp://'],
               'off': [(0, 1), (1, 2), (2, 11), (12, 21), (22, 25), (25, 31)]}]}
     ),
    # emoticon
    (':-) A:-( :). B:smile::sad: C:):(! :)., :-))) :---( Hi:).',
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': [':-)', 'A', ':-(', ':)', '.'], 'off': [(0, 3), (4, 5), (5, 8), (9, 11), (11, 12)]},
    ...
    ('[a](1)(1.a)[11.22.a.33](A.1)[a1][hello]{22}',
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['[a]', '(1)', '(1.a)', '[11.22.a.33]', '(A.1)', '[a1]', '[', 'hello', ']', '{22}'],
               'off': [(0, 3), (3, 6), (6, 11), (11, 23), (23, 28), (28, 32), (32, 33), (33, 38), (38, 39), (39, 43)]}]}

     ),
    ("don't he's can't does'nt 0's DON'IN ab'cd",
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['do', "n't", 'he', "'s", 'ca', "n't", 'does', "'nt", "0's", "DON'IN", "ab'cd"],
...
@pytest.mark.parametrize('input, expected', [
    ...
    ('#happy2018,@Jinho_Choi: ab@cde #1 $1 ',
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['#happy2018', ',', '@Jinho_Choi', ':', 'ab@cde', '#', '1', '$', '1'],
...
@pytest.mark.parametrize('input, expected', [
    ...
    ("'CAUSE 'tis ' em 'happy",
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ["'CAUSE", "'tis", "'em", "'", 'happy'],
               'off': [(0, 6), (7, 11), (12, 16), (17, 18), (18, 23)]}]}),
    # abbreviation
    ('A.B. a.B.c AB.C. Ph.D. 1.2. A-1.',
     {'doc_id': 0, 'sens': [{'tok': ['A.B.', 'a.B.c', 'AB.C', '.'], 'off': [(0, 4), (5, 10), (11, 15), (15, 16)], 'sid': 0},
               {'tok': ['Ph.D.', '1.2.', 'A-1.'], 'off': [(17, 22), (23, 27), (28, 32)], 'sid': 1}]}
     ),
    # acronym
    ('ab&cd AB|CD 1/2 a-1 1-2',
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['ab&cd', 'AB|CD', '1/2', 'a-1', '1-2'],
    ...
    # No. 1
    ('No. 1 No. a No.',
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['No.', '1', 'No', '.'], 'off': [(0, 3), (4, 5), (6, 8), (8, 9)]},
...
@pytest.mark.parametrize('input, expected', [
    ...
    ('20mg 100cm 1st 11a.m. 10PM',
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['20', 'mg', '100', 'cm', '1st', '11', 'a.m.', '10', 'PM'],
               'off': [(0, 2), (2, 4), (5, 8), (8, 10), (11, 14), (15, 17), (17, 21), (22, 24), (24, 26)]}]}
     ),
    # concatenated word
    ("whadya DON'CHA",
     {'doc_id': 0, 'sens': [{'sid': 0, 'tok': ['wha', 'd', 'ya', 'DO', "N'", 'CHA'], 'off': [(0, 3), (3, 4), (4, 6), (7, 9), (9, 11), (11, 14)]}]}
     ),
    # final mark
    ('Mbaaah.Please hello.!?world',
     {'doc_id': 0, 'sens': [{'tok': ['Mbaaah', '.'], 'off': [(0, 6), (6, 7)], 'sid': 0}, {'tok': ['Please', 'hello', '.!?'], 'off': [(7, 13), (14, 19), (19, 22)], 'sid': 1},
